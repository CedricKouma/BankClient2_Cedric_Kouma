        ...
        if (source instanceof Connexion) {
            ...
            switch (typeEvenement) {
                ...
                case "HIST": //Le serveur a renvoyé
                    ...
                    panneauHistorique.ecrireHistorique(arg);
                    JOptionPane.showConfirmDialog(null, panneauHistorique,
                            "Historique",
...
//                    arg = evenement.getArgument();
                ...
                case "EPARGNE" :
                    arg = evenement.getArgument();
                    if (arg.trim().startsWith("NO")) {
                        JOptionPane.showMessageDialog(panneauPrincipal,"Creation Compte Epargne Échoué");
                    }
                    else {
                        str = arg.substring(arg.indexOf("OK")+2).trim();
                ...
                case "SELECT" :
                    arg = evenement.getArgument();
                    if (arg.trim().startsWith("NO")) {
                        JOptionPane.showMessageDialog(panneauPrincipal,"Selection Compte Échoué");
                    }
                    else {
                        String restant = arg.substring(arg.indexOf("OK") + 2).trim();

                        String[] numCompteEtSolde = restant.split(" ");

                        String solde = numCompteEtSolde[numCompteEtSolde.length - 1];
                        JLabel lblSolde = panneauPrincipal.getPanneauOperationsCompte().getLblSolde();
